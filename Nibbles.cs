    ...
    public static class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                PlayMusic("T160O1L8CDEDCDL4ECC", true);
                SparklePause();
        ...
        private static int MusicTempo = 120;
        private static int MusicOctave = 0;
        private static int MusicNoteLength = 1;
        private static string[] MusicNotes1 = new[] { "c", "c#", "d", "d#", "e", "f", "f#", "g", "g#", "a", "a#", "b" };
        private static string[] MusicNotes2 = new[] { "c", "d-", "d", "e-", "e", "f", "g-", "g", "a-", "a", "b-", "b" };
        private static void PlayMusic(string music, bool background)
        {
            var thread = new Thread(() =>
            {
                ...
                while (input.Length > 0)
                {
                    ...
                    {
                        MusicOctave = Convert.ToInt32(m.Groups[1].Value);
                        if (MusicOctave < 0 || MusicOctave > 6)
                    ...
                    else if ((m = Regex.Match(input, @"^\s*([abcdefg])\s*([#\+\-])?\s*(\.)?\s*", RegexOptions.Singleline | RegexOptions.IgnoreCase)).Success)
                    {
                        ...
                        var noteInOctave = Array.IndexOf(MusicNotes1, noteStr);
                        if (noteInOctave == -1)
                            noteInOctave = Array.IndexOf(MusicNotes2, noteStr);
                        if (noteInOctave == -1)
        ...
        private static void PlayNibbles()
        {
            ...
            do
            {
                ...
                PlayMusic("T160O1>L20CDEDCDL10ECC", true);
